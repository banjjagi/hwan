...
# DB_jigup 테이블 만들기
	...
	select * from jigup.db_jigup_new where AccOwner="";
...
# DB_jigup_add(추가) 테이블 만들기
	...
	LOAD DATA LOCAL INFILE  'D:/DB/jigup_add.csv'
	INTO TABLE jigup.db_jigup_new
	FIELDS TERMINATED BY ',' 
	ENCLOSED BY '"'
	LINES TERMINATED BY '\r\n';
...
# 테스트용 db_jigup_new1 만들기
	create table jigup.db_jigup_new1 like jigup.db_jigup_new;
	insert into jigup.db_jigup_new1 select * from jigup.db_jigup_new;
...
# 텍스트 접수관련 테이블 만들기
	...
	WHERE (((db_jigup_new1.sort_jupsu) In ("유선접수","서면접수","팩스접수(2015년 이전)","인터넷접수","방문접수","고객센터","고객센터이관","사업장팩스","가입자팩스","경리단"))) and (db_unpaid.rrno_pat not in(select rrno_pat from rrno_dead))
	ORDER BY db_jigup_new1.pay_date DESC);

	# 연계용 수진자_예금주 텍스트 파일 내보내기
	SELECT * INTO OUTFILE "D:/db/link_pat_acc.txt"
		FIELDS TERMINATED BY '' 
		OPTIONALLY ENCLOSED BY '"'
		LINES TERMINATED BY "\n"
		FROM jigup.link_pat_acc;
	...
	SELECT distinct Num_Dec1, jung,Ent, rrno_pat1, rrno_pat_seq1,name_pat,rrno_ins1,rrno_ins_seq1,name_ins,amt,bank_id,account,AccOwner,rrno_Acc1
		INTO OUTFILE "D:/db/link_ins.csv"
		FIELDS TERMINATED BY ',' 
		LINES TERMINATED BY "\n"
		FROM jigup.link;

	# 증번호 텍스트접수
	insert into jigup.link(
	SELECT  distinct concat(right(jigup.db_unpaid.Num_Dec,1),substring(jigup.db_unpaid.Num_Dec,17,4),left(jigup.db_unpaid.Num_Dec,6),51,substring(jigup.db_unpaid.Num_Dec,11,6)) as Num_Dec1, 
		jigup.db_unpaid.jung_last, jigup.db_unpaid.Ent_Dec1 as ent, Replace(db_jigup_new1.rrno_pat,"-","") AS rrno_pat1, 
		"00" AS rrno_pat_seq1, jigup.db_unpaid.name_pat, Replace(db_jigup_new1.rrno_ins,"-","") AS rrno_ins1, 
		"00" AS rrno_ins_seq1, jigup.db_unpaid.name_ins, jigup.db_unpaid.amt_last as amt, db_jigup_new1.bank_id, db_jigup_new1.account, 
		db_jigup_new1.AccOwner, Replace(db_jigup_new1.rrno_ins,"-","") AS rrno_Acc1
		FROM jigup.db_unpaid INNER JOIN db_jigup_new1 ON jigup.db_unpaid.jung_last = db_jigup_new1.jung
		WHERE (((db_jigup_new1.sort_jupsu) In ("유선접수","서면접수","팩스접수(2015년 이전)","인터넷접수","방문접수","고객센터","고객센터이관","사업장팩스","가입자팩스","경리단")))
		ORDER BY db_jigup_new1.pay_date DESC);
	...
	LOAD DATA LOCAL INFILE  'D:/DB/EDI_Err.csv'
