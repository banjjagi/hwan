...
Loop, parse, jisa, `,
{
	...
	; 타이머설정, 검은색나올때까지, 100초 넘으면 넘어가기 
	settimer,Search,100000
	...
	search:
	while(true){
		pixelsearch xx,xy,750,400,1000,500,0xFFFFFF,5,FastRGB  ;123,321좌표에서 0x000000이 나타날때까지 계속 찾는다.
		if ErrorLevel=0 
			break
		if Errorlevel=1 
			continue
...
Loop, parse, jisa, `,
{
	...
	SetKeyDelay, -1; 딜레이 줄이기
	
	
	Run, notepad.exe , , , outputPid                           ; 실행되는 프로세스 번호가 변수 outputPid 에 저장.
	WinWait, ahk_class Notepad ahk_pid %outputPid%
	
	controlsend,,%A_LoopField%
	
		winTitle:="계산기"
		WinWait, %winTitle%
		SetTitleMatchMode, 3   ; 정확히 일치하는 모드이어야  아래 +가 제대로 클릭되며,  만약 사용하지 않으면  +/-키가 클릭됨.
		ControlClick, C, %winTitle% 
		ControlClick, 1, %winTitle% 
		ControlClick, 2, %winTitle% 
		ControlClick, 8, %winTitle% 
